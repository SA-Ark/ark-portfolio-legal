...
export default function TemplatesPage() {
  return (
    <div className="space-y-8">
      <section>
        <Badge variant="purple" className="mb-4">Document Generator</Badge>
        <h1 className="font-heading text-4xl font-semibold tracking-tight text-gradient">Template picker and generated preview</h1>
      ...
      <section className="grid gap-5 lg:grid-cols-[22rem_1fr]">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><ClipboardList className="h-5 w-5 text-blue-200" /> Template picker</CardTitle>
            <CardDescription>Reusable practice-area documents.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {templates.map((template, index) => (
              <div key={template.name} className={`rounded-2xl border p-4 ${index === 1 ? "border-blue-300/40 bg-blue-500/10" : "border-white/10 bg-white/[0.03]"}`}>
        ...
        <div className="space-y-5">
          <Card>
            ...
            <CardContent className="grid gap-4 md:grid-cols-2">
              <div><label className="mb-2 block text-sm text-zinc-400">Client</label><Input defaultValue="Northstar Foods" /></div>
              <div><label className="mb-2 block text-sm text-zinc-400">Matter type</label><Select defaultValue="litigation"><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="litigation">Commercial Litigation</SelectItem><SelectItem value="corporate">Corporate Transaction</SelectItem><SelectItem value="privacy">Privacy / Cybersecurity</SelectItem></SelectContent></Select></div>
              <div><label className="mb-2 block text-sm text-zinc-400">Lead attorney</label><Input defaultValue="Eleanor Voss" /></div>
              <div><label className="mb-2 block text-sm text-zinc-400">Fee structure</label><Input defaultValue="Hourly, monthly invoice, evergreen retainer" /></div>
              <div className="md:col-span-2"><label className="mb-2 block text-sm text-zinc-400">Scope notes</label><Textarea defaultValue="Representation in commercial dispute involving cold-chain logistics contract, document discovery, mediation, dispositive motion practice, and trial readiness." /></div>
              <div className="md:col-span-2 flex flex-wrap gap-3"><Button><FileSignature className="h-4 w-4" /> Generate document</Button><Button variant="secondary"><Download className="h-4 w-4" /> Export DOCX</Button></div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="rounded-2xl border border-white/10 bg-zinc-950/70 p-8 shadow-2xl">
                <div className="mx-auto max-w-3xl rounded bg-zinc-100 p-8 text-zinc-950">
                  ...
                  <div className="mt-1 text-center text-sm text-zinc-600">Engagement Letter · Attorney-Review Draft</div>
                  <div className="mt-8 text-sm leading-7">
                    ...
                    <div className="mt-10 grid grid-cols-2 gap-8 text-sm"><div><div className="border-t border-zinc-400 pt-2">Silverstone Legal LLP</div></div><div><div className="border-t border-zinc-400 pt-2">Northstar Foods</div></div></div>
                  </div>
                </div>
              </div>
            </CardContent>
